...
MODELS = [
    {"id":"cf","chapter":"classic","stage":"经典","name":"协同过滤","idea":"从 user–item 共现估计相似度，以邻域投票召回或打分。","pros":"无需内容特征；解释直观；强基线。","cons":"稀疏和冷启动；热门偏置；全量相似度成本。","metric":"Recall@K / Coverage","notebook":"4_2_collaborative_filtering"},
    {"id":"mf","chapter":"classic","stage":"经典","name":"MF","idea":"把交互矩阵分解为用户与物品低维向量，内积重构偏好。","pros":"压缩稀疏矩阵；检索友好；易加入偏置。","cons":"依赖 ID；难用上下文和内容；内积表达受限。","metric":"RMSE / Recall@K","notebook":"4_3_matrix_factorization"},
    {"id":"fm","chapter":"classic","stage":"经典/排序","name":"FM","idea":"用特征隐向量内积参数化二阶交叉，线性时间计算全部 pair。","pros":"稀疏特征共享统计；少做手工交叉。","cons":"主要是二阶；不建模行为次序。","metric":"AUC / LogLoss","notebook":"4_4_factorization_machine"},
    {"id":"gbdtlr","chapter":"classic","stage":"排序","name":"GBDT+LR","idea":"树学习非线性分桶与组合，叶节点 one-hot 交给 LR 校准。","pros":"表格特征强；可解释；服务稳定。","cons":"两阶段；叶子空间膨胀且易陈旧。","metric":"AUC / LogLoss","notebook":"4_5_gbdt_lr"},
    {"id":"word2vec","chapter":"classic","stage":"经典/召回","name":"word2vec / Item2Vec","idea":"把行为序列当句子，用 Skip-gram 学物品向量，向量近邻做全库召回。","pros":"向量可预计算；ANN 召回；保留局部次序。","cons":"冷启动物品无向量；短序列不稳；共现继承热门偏置。","metric":"Recall@K / Coverage","notebook":"4_6_word2vec"},
    {"id":"dssm","chapter":"retrieval","stage":"召回","name":"DSSM 双塔","idea":"用户塔、物品塔独立编码，同空间内积训练并用 ANN 全库检索。","pros":"item 可预计算；延迟低；可扩到亿级目录。","cons":"打分前无深交互；负采样偏差；单向量混兴趣。","metric":"Recall@K / MRR","notebook":"5_2_dssm"},
    {"id":"mind","chapter":"retrieval","stage":"召回","name":"MIND","idea":"动态路由把历史聚成多个兴趣胶囊，每个兴趣分别检索。","pros":"缓解兴趣混叠；覆盖多意图与长尾。","cons":"兴趣数敏感；多路 ANN 合并成本。","metric":"Recall@K / Coverage","notebook":"5_3_mind"},
    {"id":"deepfm","chapter":"ranking","stage":"排序","name":"DeepFM","idea":"FM 一二阶分支与 DNN 高阶分支共享 embedding 并联合训练。","pros":"端到端；兼顾记忆与泛化；成熟基线。","cons":"高阶交互隐式；无序列和去偏专门机制。","metric":"AUC / LogLoss","notebook":"6_2_deepfm"},
    {"id":"din","chapter":"ranking","stage":"排序","name":"DIN","idea":"用候选感知 attention 激活与当前物品相关的历史。","pros":"同一用户面对不同候选产生不同表示。","cons":"逐候选读取历史，延迟随候选数增长。","metric":"AUC / GAUC / P99","notebook":"6_3_din"},
    {"id":"dien","chapter":"ranking","stage":"排序","name":"DIEN","idea":"用辅助损失、GRU 与 AUGRU 显式学习兴趣状态及其演化。","pros":"利用顺序与兴趣变化；监督更密集。","cons":"训练与服务更复杂，校准和时延压力更高。","metric":"AUC / LogLoss / P99","notebook":"6_4_dien"},
    {"id":"mmoe","chapter":"multitask","stage":"多目标","name":"MMoE","idea":"每个任务用独立 gate 混合同一组共享专家。","pros":"共享灵活；适合相关任务。","cons":"专家塌缩、梯度冲突与样本空间错位。","metric":"逐任务 AUC + business value","notebook":"7_2_mmoe"},
    {"id":"ple","chapter":"multitask","stage":"多目标","name":"PLE","idea":"逐层分离共享与任务专属专家，控制知识共享边界。","pros":"缓解负迁移与跷跷板。","cons":"结构、参数和调参成本更高。","metric":"逐任务 AUC + transfer gap","notebook":"7_3_ple"},
    {"id":"sasrec","chapter":"retrieval","stage":"召回/Transformer","name":"SASRec","idea":"以因果自注意力编码真实行为序列，输出当前用户向量并预测下一物品。","pros":"并行训练；兼顾近期转移与较长依赖；可接全库检索。","cons":"注意力随序列长度平方增长；严格依赖时间协议。","metric":"HR@K / NDCG@K / P99","notebook":"5_4_sasrec"},
    {"id":"openonerec","chapter":"generative","stage":"生成式","name":"OpenOneRec","idea":"将 item/list token 化，自回归生成并用目录约束和奖励对齐。","pros":"统一列表目标；支持偏好优化。","cons":"无效 ID、重复、解码延迟与目录更新。","metric":"NDCG / invalid rate / P99","notebook":"8_2_openonerec_practice"},
    {"id":"hstu","chapter":"generative","stage":"生成式","name":"DLRM HSTU","idea":"把长期行为流作为统一序列进行 next-item 预训练与下游迁移。","pros":"长序列、统一表征和规模效应。","cons":"训练服务协同复杂，GPU ROI 要求高。","metric":"HR/NDCG + throughput / P99","notebook":"8_3_dlrm_hstu_practice"},
]
...
DATASETS = [
    ...
    {"name":"RecIF-Bench","year":"2025","scale":"100M interactions, 200K users","fit":"8.2 OpenOneRec 生成式召排","note":"短视频/广告/商品统一评测；遵循官方许可","url":"https://github.com/Kuaishou-OneRec/OpenOneRec"},
]
...
NOTEBOOKS = [
    {"slug":"3_1_math_foundations","title":"3.1 基础课程总览：从数据语义到模型训练","kind":"foundation","framework":"NumPy / Matplotlib","dataset":"MovieLens latest-small + 手算投影","runtime":"约 15 秒 CPU"},
    {"slug":"3_2_data_ml_basics","title":"3.2 数据与机器学习基础：一行数据到底代表什么","kind":"curriculum","framework":"pandas / NumPy","dataset":"MovieLens latest-small 真实行为 + 教学拆分","runtime":"约 15 秒 CPU"},
    {"slug":"3_3_linear_algebra","title":"3.3 线性代数：把推荐数据放进有形状的盒子","kind":"curriculum","framework":"NumPy / Matplotlib","dataset":"明确标注的数学教学数字","runtime":"约 15 秒 CPU"},
    {"slug":"3_4_calculus","title":"3.4 微积分：参数动一点，损失会怎样","kind":"curriculum","framework":"NumPy / Matplotlib","dataset":"明确标注的数学教学数字","runtime":"约 15 秒 CPU"},
    {"slug":"3_5_probability_statistics","title":"3.5 概率与统计：把不确定性说清楚","kind":"curriculum","framework":"NumPy / Matplotlib","dataset":"明确标注的数学教学数字","runtime":"约 15 秒 CPU"},
    {"slug":"3_6_information_theory","title":"3.6 信息论：一次意外有多少信息","kind":"curriculum","framework":"NumPy / Matplotlib","dataset":"明确标注的数学教学数字","runtime":"约 15 秒 CPU"},
    {"slug":"3_7_optimization","title":"3.7 优化：怎样让模型稳定地下山","kind":"curriculum","framework":"NumPy / Matplotlib","dataset":"明确标注的数学教学数字","runtime":"约 15 秒 CPU"},
    {"slug":"3_8_data_pipeline","title":"3.8 数据与实验基础：从 import 到训练循环","kind":"foundation","framework":"pandas / PyTorch / inspect","dataset":"MovieLens latest-small 真实行为","runtime":"约 20 秒 CPU"},
    {"slug":"4_1_classic_foundations","title":"4.1 导读与数学基础：经典算法","kind":"foundation","framework":"NumPy / Matplotlib","dataset":"MovieLens latest-small","runtime":"约 10 秒 CPU"},
    {"slug":"4_2_collaborative_filtering","title":"4.2 协同过滤：UserCF / ItemCF","kind":"algorithm","framework":"NumPy","dataset":"MovieLens latest-small","runtime":"约 20 秒 CPU"},
    {"slug":"4_3_matrix_factorization","title":"4.3 矩阵分解：BiasMF","kind":"algorithm","framework":"PyTorch","dataset":"MovieLens latest-small","runtime":"约 30 秒 CPU"},
    {"slug":"4_4_factorization_machine","title":"4.4 因子分解机：FM","kind":"algorithm","framework":"PyTorch / scikit-learn","dataset":"MovieLens latest-small 评分偏好任务","runtime":"约 30 秒 CPU"},
    {"slug":"4_5_gbdt_lr","title":"4.5 GBDT+LR","kind":"algorithm","framework":"XGBoost / scikit-learn","dataset":"MovieLens latest-small 评分偏好任务","runtime":"约 30 秒 CPU"},
    {"slug":"4_6_word2vec","title":"4.6 word2vec / Item2Vec","kind":"algorithm","framework":"PyTorch","dataset":"MovieLens latest-small 行为序列","runtime":"约 25 秒 CPU"},
    {"slug":"4_7_classic_summary","title":"4.7 总结：经典算法横向对比","kind":"summary","framework":"结果聚合与选型","dataset":"MovieLens 真实实验 JSON","runtime":"约 10 秒 CPU"},
    {"slug":"5_1_retrieval_foundations","title":"5.1 导读与数学基础：向量召回","kind":"foundation","framework":"NumPy / Matplotlib","dataset":"full：各论文公开协议；smoke：Amazon Reviews 2023","runtime":"约 10 秒 CPU"},
    {"slug":"5_2_dssm","title":"5.2 DSSM 双塔召回","kind":"algorithm","framework":"Torch-RecHub DSSM","dataset":"full：Amazon Books 5-core 迁移；smoke：Amazon 真实切片","runtime":"约 20 秒 CPU"},
    {"slug":"5_3_mind","title":"5.3 MIND 多兴趣召回","kind":"algorithm","framework":"Torch-RecHub MIND","dataset":"full：Amazon Books 2014 10-core；smoke：Amazon 真实切片","runtime":"约 20 秒 CPU"},
    {"slug":"5_4_sasrec","title":"5.4 SASRec 序列召回","kind":"algorithm","framework":"Torch-RecHub SASRec","dataset":"full：MovieLens-1M 论文协议；smoke：Amazon 真实切片","runtime":"约 35 秒 CPU"},
    {"slug":"5_5_retrieval_summary","title":"5.5 总结：DSSM、MIND 与 SASRec","kind":"summary","framework":"结果聚合与选型","dataset":"full：按算法分别审计；smoke：Amazon 2023 实验 JSON","runtime":"约 5 秒 CPU"},
    {"slug":"6_1_ranking_foundations","title":"6.1 导读与数学基础：CTR 排序","kind":"foundation","framework":"NumPy / Matplotlib","dataset":"full：Criteo / Amazon Electronics；smoke：KuaiRand","runtime":"约 10 秒 CPU"},
    {"slug":"6_2_deepfm","title":"6.2 DeepFM 排序","kind":"algorithm","framework":"Torch-RecHub DeepFM","dataset":"full：Criteo；smoke：KuaiRand-Pure is_click","runtime":"约 20 秒 CPU"},
    {"slug":"6_3_din","title":"6.3 DIN 候选感知排序","kind":"algorithm","framework":"Torch-RecHub DIN","dataset":"full：Amazon Electronics；smoke：KuaiRand feed 序列","runtime":"约 25 秒 CPU"},
    {"slug":"6_4_dien","title":"6.4 DIEN 兴趣演化排序","kind":"algorithm","framework":"Torch-RecHub DIEN","dataset":"full：Amazon Electronics；smoke：KuaiRand feed 序列","runtime":"约 40 秒 CPU"},
    {"slug":"6_5_ranking_summary","title":"6.5 总结：DeepFM、DIN 与 DIEN","kind":"summary","framework":"结果聚合与选型","dataset":"full：按算法分别审计；smoke：KuaiRand 实验 JSON","runtime":"约 5 秒 CPU"},
    {"slug":"7_1_multitask_foundations","title":"7.1 导读与数学基础：多目标学习","kind":"foundation","framework":"NumPy / Matplotlib","dataset":"full：Census-Income；smoke：KuaiRand 多反馈","runtime":"约 10 秒 CPU"},
    {"slug":"7_2_mmoe","title":"7.2 MMoE 多目标学习","kind":"algorithm","framework":"Torch-RecHub MMOE","dataset":"full：Census-Income；smoke：KuaiRand 双目标","runtime":"约 20 秒 CPU"},
    {"slug":"7_3_ple","title":"7.3 PLE 渐进式专家抽取","kind":"algorithm","framework":"Torch-RecHub PLE","dataset":"full：Census-Income；smoke：KuaiRand 双目标","runtime":"约 25 秒 CPU"},
    {"slug":"7_4_multitask_summary","title":"7.4 总结：MMoE 与 PLE","kind":"summary","framework":"结果聚合与选型","dataset":"full：Census-Income；smoke：KuaiRand 实验 JSON","runtime":"约 5 秒 CPU"},
    {"slug":"8_1_generative_foundations","title":"8.1 导读与数学基础：生成式推荐","kind":"foundation","framework":"NumPy / Matplotlib","dataset":"full：按算法官方协议；smoke：KuaiRand 适配器","runtime":"默认 CUDA；CPU 仅基础检查"},
    {"slug":"8_2_openonerec_practice","title":"8.2 OpenOneRec 实战","kind":"algorithm","framework":"OpenOneRec 契约 + PyTorch 约束生成器","dataset":"KuaiRand taxonomy adapter；RecIF full 需授权","runtime":"CUDA 混合精度；CPU basic smoke"},
    {"slug":"8_3_dlrm_hstu_practice","title":"8.3 DLRM HSTU 实战","kind":"algorithm","framework":"Torch-RecHub HSTU + Meta DLRM-v3 配置","dataset":"full：Meta MovieLens-20M + Amazon Books；smoke：KuaiRand feed 序列","runtime":"CUDA 混合精度；官方配置需 4×24GB GPU"},
    {"slug":"8_4_generative_summary","title":"8.4 总结：生成式召回、排序与召排融合","kind":"summary","framework":"结果聚合与工业选型","dataset":"full：按算法分别审计；smoke：KuaiRand 实验 JSON","runtime":"默认 CUDA；CPU 仅阅读"},
]
...
MATH_PREREQUISITES = [
    ...
    _math_topic(
        ...
        "全部算法的实验协议与结果比较", "3_2_data_ml_basics", "split-leakage",
    ...
    _math_topic(
        ...
        "embedding、行为序列、多兴趣与多任务批次", "3_3_linear_algebra", "tensors-shapes", (),
    ...
    _math_topic(
        ...
        "CF 相似度、MF/FM 交互、双塔与候选匹配", "3_3_linear_algebra", "elementwise-dot",
    ...
    _math_topic(
        ...
        "MF/FM、word2vec、双塔、排序网络与生成模型", "3_3_linear_algebra", "matmul-embedding",
    ...
    _math_topic(
        ...
        "MF、多兴趣路由、SASRec、DIN/DIEN 与 HSTU", "3_3_linear_algebra", "low-rank-attention",
    ...
    _math_topic(
        ...
        "LR、FM、深度排序、序列与生成模型", "3_4_calculus", "functions", (),
    ...
    _math_topic(
        ...
        "除邻域 CF 外所有可训练参数的更新", "3_4_calculus", "derivative-gradient",
        ("calculus-functions",),
    ...
    _math_topic(
        ...
        "FM 与全部神经网络的自动求导", "3_4_calculus", "chain-rule",
    ...
    _math_topic(
        ...
        "反馈建模、负采样、CTR 与序列预测", "3_5_probability_statistics", "random-variable", (),
        tuple(_MODEL_NOTEBOOK_BY_ID),
    ),
    _math_topic(
        "probability-conditional-chain", "概率与统计", "联合、边缘、条件概率与链式分解",
        "条件概率是在已知用户或历史之后重新计算可能性；一段序列的联合概率可拆成从左到右的一串条件概率。",
        "召回条件概率、多任务漏斗与自回归生成", "3_5_probability_statistics", "conditional-chain",
    ...
    _math_topic(
        ...
        "指标汇总、mini-batch 训练与稳定性判断", "3_5_probability_statistics", "expectation-variance",
    ...
    _math_topic(
        ...
        "GBDT+LR、CTR 模型、负采样召回与多任务概率", "3_5_probability_statistics", "likelihood-calibration",
    ...
    _math_topic(
        ...
        "分类不确定性、推荐分布与归一化指标", "3_6_information_theory", "information-entropy",
        ("probability-random-variable",),
    ...
    _math_topic(
        ...
        "CTR LogLoss、sampled softmax、分类与分布对齐", "3_6_information_theory", "cross-entropy-kl",
    ...
    _math_topic(
        ...
        "召回分类、多兴趣路由、注意力与受控采样", "3_6_information_theory", "softmax-temperature",
    ...
    _math_topic(
        ...
        "SASRec/HSTU 的逐位置目标与 OpenOneRec 偏好优化", "3_6_information_theory", "sequence-nll-dpo",
    ...
    _math_topic(
        ...
        "MF/FM、embedding、深度与生成模型训练", "3_7_optimization", "sgd",
    ...
    _math_topic(
        ...
        "全部梯度训练算法的收敛速度与稳定性", "3_7_optimization", "learning-rate",
        ("optimization-sgd",),
        ("mf", "fm", "word2vec", "dssm", "mind", "sasrec", "deepfm", "din", "dien", "mmoe", "ple", "openonerec", "hstu"),
    ),
    _math_topic(
        ...
        "MF/FM、深度排序、序列与生成模型", "3_7_optimization", "regularization",
    ...
    _math_topic(
        ...
        "MMoE 与 PLE 的共享边界和跷跷板现象", "3_7_optimization", "gradient-conflict",
...
CHAPTERS = {
    "foundations": {
        "number": "3", "title": "共性基础课程：从数据语义到实验管线",
        "intro": "面向高中毕业生，从一行数据的含义开始，循序学习线性代数、微积分、概率统计、信息论损失与优化，再把这些概念接入可执行训练管线。",
        "layout": "总览负责路线与最小直觉；3.2—3.7 各自完整讲解一组共性知识；3.8 打开项目代码，串起时间切分、张量化、训练、推理、测试与结果落盘。",
        "use_cases": "这是第 4—8 章的共同先修课程。算法页只展开论文新增数学，其余概念精确回链到第 3 章的稳定锚点。",
        "model_ids": [],
        "opening": "3_1_math_foundations",
        "notebooks": [
    ...
    "classic": {
        "number": "4", "title": "经典算法：协同过滤、MF、FM、GBDT+LR、word2vec",
        ...
        "opening": "4_1_classic_foundations",
        "notebooks": ["4_2_collaborative_filtering", "4_3_matrix_factorization", "4_4_factorization_machine", "4_5_gbdt_lr", "4_6_word2vec"],
        "summary": "4_7_classic_summary",
        "sources": [
    ...
    "retrieval": {
        "number": "5", "title": "召回模型：双塔、多兴趣与序列召回",
        ...
        "model_ids": ["dssm", "mind", "sasrec"], "opening": "5_1_retrieval_foundations", "notebooks": ["5_2_dssm", "5_3_mind", "5_4_sasrec"], "summary": "5_5_retrieval_summary",
    ...
    "ranking": {
        "number": "6", "title": "排序模型：DeepFM、DIN 与 DIEN",
        ...
        "model_ids": ["deepfm", "din", "dien"], "opening": "6_1_ranking_foundations", "notebooks": ["6_2_deepfm", "6_3_din", "6_4_dien"], "summary": "6_5_ranking_summary",
    ...
    "multitask": {
        "number": "7", "title": "多目标模型：MMoE 与 PLE",
        ...
        "model_ids": ["mmoe", "ple"], "opening": "7_1_multitask_foundations", "notebooks": ["7_2_mmoe", "7_3_ple"], "summary": "7_4_multitask_summary",
    ...
    "generative": {
        "number": "8", "title": "生成式推荐：生成召回、排序与召排融合",
        ...
        "model_ids": ["openonerec", "hstu"], "opening": "8_1_generative_foundations", "notebooks": ["8_2_openonerec_practice", "8_3_dlrm_hstu_practice"], "summary": "8_4_generative_summary",
...
def notebook_has_paper_guide(slug: str) -> bool:
    ...
    The foundations chapter (3) and every chapter opening / 导读 page focus on
