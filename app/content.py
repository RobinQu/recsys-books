...
MODELS = [
    ...
    {"id":"sasrec","chapter":"transformer","stage":"序列/Transformer","name":"SASRec","idea":"以因果自注意力编码真实行为序列，在每个位置预测下一物品。","pros":"并行训练；兼顾近期转移与较长依赖。","cons":"注意力随序列长度平方增长；严格依赖时间协议。","metric":"HR@K / NDCG@K / P99","notebook":"3_5_1_sasrec"},
...
DATASETS = [
    {"name":"MovieLens latest-small","year":"2018 固定版本","scale":"100,836 ratings · 610 users · 9,742 movies","fit":"本教程全部 27 本 Notebook 的默认真实数据底座","note":"官方 CSV 与许可随仓库保存；真实时间戳切分，派生标签规则公开","url":"https://files.grouplens.org/datasets/movielens/ml-latest-small-README.html"},
    {"name":"Amazon Reviews 2023","year":"2023","scale":"571.54M reviews, 54.51M users, 48.19M items","fit":"序列召回、排序、多模态冷启","note":"按品类与 5-core 下载；时间切分","url":"https://amazon-reviews-2023.github.io/"},
...
NOTEBOOKS = [
    {"slug":"3_0_math_foundations","title":"3.0 推荐算法数学基础：从矩阵到梯度下降","framework":"NumPy / Matplotlib","dataset":"MovieLens latest-small + 手算投影","runtime":"约 15 秒 CPU"},
    ...
    {"slug":"3_2_0_retrieval_foundations","title":"3.2 导读与数学基础：向量召回","framework":"NumPy / Matplotlib","dataset":"MovieLens latest-small","runtime":"约 10 秒 CPU"},
    {"slug":"3_2_1_dssm","title":"3.2.1 DSSM 双塔召回","framework":"Torch-RecHub DSSM","dataset":"MovieLens latest-small 时间留出","runtime":"约 20 秒 CPU"},
    {"slug":"3_2_2_mind","title":"3.2.2 MIND 多兴趣召回","framework":"Torch-RecHub MIND","dataset":"MovieLens latest-small 真实序列","runtime":"约 20 秒 CPU"},
    {"slug":"3_2_summary","title":"3.2 总结：DSSM 与 MIND","framework":"结果聚合与选型","dataset":"MovieLens 真实实验 JSON","runtime":"约 5 秒 CPU"},
    {"slug":"3_3_0_ranking_foundations","title":"3.3 导读与数学基础：CTR 排序","framework":"NumPy / Matplotlib","dataset":"MovieLens latest-small","runtime":"约 10 秒 CPU"},
    {"slug":"3_3_1_deepfm","title":"3.3.1 DeepFM 排序","framework":"Torch-RecHub DeepFM","dataset":"MovieLens latest-small 评分偏好任务","runtime":"约 20 秒 CPU"},
    {"slug":"3_3_2_din","title":"3.3.2 DIN 候选感知排序","framework":"Torch-RecHub DIN","dataset":"MovieLens latest-small 真实序列","runtime":"约 25 秒 CPU"},
    {"slug":"3_3_3_dien","title":"3.3.3 DIEN 兴趣演化排序","framework":"Torch-RecHub DIEN","dataset":"MovieLens latest-small 真实序列","runtime":"约 40 秒 CPU"},
    {"slug":"3_3_summary","title":"3.3 总结：DeepFM、DIN 与 DIEN","framework":"结果聚合与选型","dataset":"MovieLens 真实实验 JSON","runtime":"约 5 秒 CPU"},
    {"slug":"3_4_0_multitask_foundations","title":"3.4 导读与数学基础：多目标学习","framework":"NumPy / Matplotlib","dataset":"MovieLens latest-small","runtime":"约 10 秒 CPU"},
    {"slug":"3_4_1_mmoe","title":"3.4.1 MMoE 多目标学习","framework":"Torch-RecHub MMOE","dataset":"MovieLens latest-small 双评分阈值","runtime":"约 20 秒 CPU"},
    {"slug":"3_4_2_ple","title":"3.4.2 PLE 渐进式专家抽取","framework":"Torch-RecHub PLE","dataset":"MovieLens latest-small 双评分阈值","runtime":"约 25 秒 CPU"},
    {"slug":"3_4_summary","title":"3.4 总结：MMoE 与 PLE","framework":"结果聚合与选型","dataset":"MovieLens 真实实验 JSON","runtime":"约 5 秒 CPU"},
    {"slug":"3_5_0_transformer_foundations","title":"3.5 导读与数学基础：Transformer 序列推荐","framework":"NumPy / Matplotlib","dataset":"MovieLens latest-small 真实时间序列","runtime":"约 10 秒 CPU"},
    {"slug":"3_5_1_sasrec","title":"3.5.1 SASRec 自注意力序列推荐","framework":"Torch-RecHub SASRec","dataset":"MovieLens latest-small 真实时间序列","runtime":"约 35 秒 CPU"},
    {"slug":"3_5_summary","title":"3.5 总结：SASRec 与 Transformer 序列推荐","framework":"结果聚合与选型","dataset":"MovieLens 真实实验 JSON","runtime":"约 5 秒 CPU"},
    {"slug":"4_0_generative_foundations","title":"4.0 导读与数学基础：生成式推荐","framework":"NumPy / Matplotlib","dataset":"MovieLens latest-small","runtime":"约 10 秒 CPU"},
    {"slug":"4_1_generative_overview","title":"4.1 总结：生成式召回、排序与融合","framework":"结果聚合与工业选型","dataset":"MovieLens 真实实验 JSON","runtime":"约 5 秒 CPU"},
    {"slug":"4_2_openonerec_practice","title":"4.2 OpenOneRec 实战","framework":"OpenOneRec 契约 + PyTorch 约束生成器","dataset":"MovieLens latest-small Semantic ID","runtime":"smoke 1 分钟；全量需 GPU"},
    {"slug":"4_3_dlrm_hstu_practice","title":"4.3 DLRM HSTU 实战","framework":"Torch-RecHub HSTU + Meta DLRM-v3 配置","dataset":"MovieLens latest-small 真实时间序列","runtime":"smoke 2 分钟；官方配置需 4×24GB GPU"},
]
...
CHAPTERS = {
    "foundations": {
        ...
        "model_ids": [], "opening": "3_0_math_foundations", "notebooks": ["3_0_math_foundations"], "summary": "3_0_math_foundations",
        "sources": [
    ...
    "retrieval": {
        "number": "3.2", "title": "召回模型：DSSM 双塔与 MIND",
        "intro": "召回的核心约束是全库可检索。DSSM 以独立编码换取 ANN；MIND 用多个兴趣向量改善单向量的兴趣混叠。",
        "layout": "先理解双塔、负采样与向量索引，再观察多兴趣抽取和多路候选合并，最后比较 Recall、Coverage 与服务成本。",
        "use_cases": "DSSM 适合亿级目录通用召回；MIND 适合长历史、多意图的电商和内容场景。",
        "model_ids": ["dssm", "mind"], "opening": "3_2_0_retrieval_foundations", "notebooks": ["3_2_1_dssm", "3_2_2_mind"], "summary": "3_2_summary",
        "sources": [("Huang et al., 2013：DSSM", "https://www.microsoft.com/en-us/research/publication/learning-deep-structured-semantic-models-for-web-search-using-clickthrough-data/", "以点击数据联合学习两侧表示，双塔的工程价值来自 item 向量可离线预计算。"), ("Li et al., 2019：MIND", "https://arxiv.org/abs/1904.08030", "动态路由从行为中抽取多个兴趣胶囊，用 label-aware attention 选择训练兴趣。")],
    },
    "ranking": {
    ...
    "transformer": {
        "number": "3.5", "title": "Transformer 序列推荐：SASRec 及其演进",
        "intro": "从真实时间序列出发，理解位置编码、因果自注意力与下一物品预测，并比较 SASRec、BERT4Rec 和 HSTU 的任务边界。",
        "layout": "先在导读中手算 Scaled Dot-Product Attention 与 causal mask，再用 Torch-RecHub SASRec 训练 MovieLens 真实序列，最后与热门基线及后续 Transformer 路线比较。",
        "use_cases": "适合点击、观看、购买等有序行为；序列很短或新鲜度要求极高时，应同时保留 Markov、热门与轻量召回基线。",
        "model_ids": ["sasrec"], "opening": "3_5_0_transformer_foundations", "notebooks": ["3_5_1_sasrec"], "summary": "3_5_summary",
        "sources": [
            ("Kang & McAuley, 2018：SASRec", "https://arxiv.org/abs/1808.09781", "用因果自注意力在每个时刻选择与下一行为相关的历史。"),
            ("Sun et al., 2019：BERT4Rec", "https://arxiv.org/abs/1904.06690", "以双向 Transformer 和 masked-item Cloze 任务学习序列表征。"),
            ("Zhai et al., 2024：HSTU", "https://arxiv.org/abs/2402.17152", "针对大规模推荐行为流重新设计序列转换单元与系统协同。"),
        ],
    },
    "generative": {
