...
DEEP_MATH_PREREQUISITES = {
    ...
    "5_4_sasrec": {
        "generic": [
            ("序列、会话与时间切分", "/notebooks/3_2_data_ml_basics#split-leakage"),
            ("矩阵乘法、embedding 与加权和", "/notebooks/3_3_linear_algebra#matmul-embedding"),
            ("Q/K/V 形状", "/notebooks/3_3_linear_algebra#low-rank-attention"),
            ("Softmax 与温度", "/notebooks/3_6_information_theory#softmax-temperature"),
        ],
    ...
    "7_3_ple": {
        "generic": [
            ...
            ("Softmax 与加权和", "/notebooks/3_6_information_theory#softmax-temperature"),
            ("多任务梯度冲突", "/notebooks/3_7_optimization#gradient-conflict"),
            ("正则化与泛化", "/notebooks/3_7_optimization#regularization"),
        ],
...
ax.set_title('PLE: shared plus task-specific experts'); plt.colorbar(image,ax=ax,ticks=[0,1]); plt.show()""",
        ...
        {
            "slug": "8_2_openonerec_practice",
            "title": "8.2 OpenOneRec：受约束列表生成实战",
            "chapter": "chapter_8",
...
ax.plot([.72,.86],[.38,.27]); ax.plot([.72,.86],[.38,.49]); ax.set_title('Catalog trie allows valid next tokens'); plt.show()""",
        ...
        {
            "slug": "8_3_dlrm_hstu_practice",
            "title": "8.3 DLRM HSTU：长行为序列实战",
            "chapter": "chapter_8",
            "function": "run_hstu",
...
ax.set_xticks(range(length)); ax.set_yticks(range(length)); plt.colorbar(image,ax=ax,ticks=[0,1]); plt.show(); print(mask.astype(int))""",
    ...
    paper_evidence = {
        ...
        "8_2_openonerec_practice": ("onerec", "原文 Table 1（离线，指标由 reward 模型估计而非真实行为）：OneRec-1B+IPA 的 swt mean/max 为 0.1025/0.1933，OneRec-1B 为 0.0991/0.1529，TIGER-1B 为 0.0873/0.1368，SASRec 仅 0.0375/0.0803；IPA 优于 DPO/IPO/cDPO/rDPO/CPO/simPO/S-DPO 全部对比变体。Table 2（快手主页面 1% 流量在线 A/B）：OneRec-1B+IPA 总观看时长 +1.68%、平均观看时长 +6.56%（摘要约写作 1.6%）。实现细节：Semantic ID 3 层、每层 8192 簇，历史 n=256、session m=5，MoE 24 专家每 token 激活 2 个，beam size 128，推理仅 13% 参数激活；0.05B→1B 规模消融持续提升。以上是快手业务口径，教程在 KuaiRand 上复现 Semantic ID、trie 约束与列表指标契约，数值不可相减。"),
        "8_3_dlrm_hstu_practice": ("hstu", "原文 Table 4（公开数据，multi-pass full-shuffle 设定）：相对论文自跑的 SASRec，ML-1M HR@10 0.2853→0.3097（+8.6%，HSTU-large +15.5%），ML-20M +11.9%（large +22.8%），Books 0.0292→0.0404（+38.4%，large +60.6%），Books NDCG@10 最高 +65.8%。Table 5（工业单遍流式）消融显示换成 softmax 注意力或去掉相对偏置均退化；编码器与 FlashAttention-2 同配置对比，训练最高快 15.2 倍、推理快 5.6 倍（H100、bfloat16、序列 1024–8192）。Table 7（工业排序在线 A/B）：GR 相对 DLRM E-Task +12.4%、C-Task +4.4%；模型质量随训练算力幂律扩展，最大 1.5 万亿参数。以上是 Meta 系统口径，轻量实验只验证接口，数值不可相减。"),
    }
...
HSTU 的论文级差异不在 next-item 目标，而在统一序列化特征、相对位置/时间偏置与不归一化的逐点聚合注意力。论文另配生成式训练采样（按 $1/n_i$ 采样用户把成本降一个 $O(N)$ 因子）与 M-FALCON 微批推理，属于工业配置，教学 Notebook 不展开。教程默认在 CUDA 上运行 Torch-RecHub 模型并启用混合精度；CPU basic smoke 只保留核心张量契约。""", "`run_hstu` 从 KuaiRand 真实 feed 流生成 next-item 样本并直接计算教学词表交叉熵；它不是 Meta full 数据/采样协议，实现映射会明确两者边界。"),
    ...
    for meta in algorithms:
        cuda_first = meta["slug"].startswith("8_")
        run_expression = (
            f"{meta['function']}(cpu_smoke=not torch.cuda.is_available(), progress=print_progress)"
            if cuda_first else f"{meta['function']}(progress=print_progress)"
        )
...
print('PASS：数据、训练、推理、测试和结果产物均已验证。')"""),
    ...
    summaries = [
        ("5_5_retrieval_summary", "5.5", "DSSM、MIND 与 SASRec 召回", "chapter_5", 3,
    ...
    paper_audits = {
        ...
        "7_4_multitask_summary": [
            {
                "algorithm": "MMoE",
                "source_notebook": "7_2_mmoe",
    ...
    relationship_intros = {
        "8_4_generative_summary": "三篇论文不是同一赛道上的升级排名，而是把“生成”逐层推深：TIGER 先回答“物品怎么变成可生成的标识”，OneRec 回答“生成能不能统一召回与排序并对齐偏好”，HSTU 回答“整个推荐能不能变成随算力扩展的序列转导”。下表中的“交接问题”比发表年份更重要。",
        "5_5_retrieval_summary": "三篇论文不是同一赛道上的简单升级排名。DSSM 提供“可独立编码并比较”的结构起点；MIND 沿着**表示数量**扩展，把一个用户拆成多个并行兴趣；SASRec 沿着**时间顺序**扩展，让当前表示随历史位置变化。下表中的“交接问题”比发表年份更重要。",
        "6_5_ranking_summary": "三篇论文沿同一条 CTR 排序线各自补一块短板：DeepFM 解决“静态特征交互要不要手工”，DIN 解决“用户表示与候选无关”，DIEN 解决“历史没有次序、行为不等于兴趣”。下表中的“交接问题”比发表年份更重要。",
        "7_4_multitask_summary": "两篇论文沿同一条多任务结构线：MMoE 解决“共享底层对任务关系太敏感”，PLE 解决“共享专家仍被无差别共享、跷跷板仍在”。下表中的“交接问题”比发表年份更重要。",
    }
    future_columns = {
        "8_4_generative_summary": ["dimension", "TIGER line", "OneRec line", "HSTU line", "next questions"],
        "5_5_retrieval_summary": ["dimension", "DSSM line", "MIND line", "SASRec line", "next questions"],
        "6_5_ranking_summary": ["dimension", "DeepFM line", "DIN line", "DIEN line", "next questions"],
        "7_4_multitask_summary": ["dimension", "MMoE line", "PLE line", "next questions"],
    }
...
print('结论：当前结果是可执行的 smoke/教学实验，不是 paper reproduction。')"""
            if slug in ("5_5_retrieval_summary", "6_5_ranking_summary", "7_4_multitask_summary"):
                audit_code = f"""paper_audit = pd.DataFrame({paper_audits[slug]!r})
