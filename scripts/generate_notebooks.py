...
def main():
    specs = {}
    specs["3_0_7_data_pipeline"] = notebook(
        "3.0.7 数据与实验基础：从 import 到训练循环",
...
"""),
    ...
    specs["3_0_math_foundations"] = notebook(
        "3.0 基础课程总览：从数据语义到模型训练",
        "用一张循序递进的路线图串起六门数学与机器学习基础课，以及最后的数据实验管线。这里建立共同语言和最小手算直觉；完整教学沿 3.0.1—3.0.7 逐课展开。",
...
3.0 负责建立所有算法共用的语言；算法页只展开论文新引入的结构与公式。建议按表格从上到下学习，遇到熟悉内容也可直接跳到对应的精确小节；六门概念课之后，再进入 [3.0.7 数据与实验基础](/notebooks/3_0_7_data_pipeline)：

| 共同领域 | 先掌握什么 | 后续主要用在 |
|---|---|---|
| 数据与机器学习 | [一行数据、特征与标签](/notebooks/3_0_1_data_ml_basics#observation-label)；[显式/隐式反馈、曝光、未知与假负例](/notebooks/3_0_1_data_ml_basics#implicit-feedback)；[切分、泄漏与泛化](/notebooks/3_0_1_data_ml_basics#split-leakage) | 全部章节的数据协议与结果边界 |
| 线性代数 | [张量形状与轴](/notebooks/3_0_2_linear_algebra#tensors-shapes)；[逐元素运算、点积与余弦](/notebooks/3_0_2_linear_algebra#elementwise-dot)；[矩阵乘法与 embedding](/notebooks/3_0_2_linear_algebra#matmul-embedding) | CF、MF、FM、双塔、注意力 |
| 微积分 | [函数与复合](/notebooks/3_0_3_calculus#functions)；[偏导与梯度](/notebooks/3_0_3_calculus#derivative-gradient)；[链式法则与反向传播](/notebooks/3_0_3_calculus#chain-rule) | 所有需要训练参数的模型 |
| 概率与统计 | [条件概率](/notebooks/3_0_4_probability_statistics#conditional-chain)；[期望与方差](/notebooks/3_0_4_probability_statistics#expectation-variance)；[odds、logit 与校准](/notebooks/3_0_4_probability_statistics#likelihood-calibration) | CTR、多任务、序列生成与实验解读 |
| 信息论 | [交叉熵与 KL](/notebooks/3_0_5_information_theory#cross-entropy-kl)；[Softmax、温度与 NE](/notebooks/3_0_5_information_theory#softmax-temperature)；[序列 NLL](/notebooks/3_0_5_information_theory#sequence-nll-dpo) | LR/GBDT、word2vec、DSSM、Transformer |
| 优化 | [SGD 与 mini-batch](/notebooks/3_0_6_optimization#sgd)；[学习率](/notebooks/3_0_6_optimization#learning-rate)；[正则化、过拟合与早停](/notebooks/3_0_6_optimization#regularization) | MF、深度模型、生成式模型 |
...
注意数据语义不能混用：星级是用户主动给出的**显式反馈**；点击/观看是行为留下的**隐式反馈**；“曝光后未点击”才是有明确分母的负标签；从未曝光或日志未记录通常只能叫**未知**。从未知项抽作负样本会产生**假负例**，必须记录采样规则。完整例子见 [3.0.1 显式、隐式反馈与未知项](/notebooks/3_0_1_data_ml_basics#implicit-feedback)。
"""),
            code(r"""
import numpy as np
import pandas as pd
...
先看**轴与形状**再看公式：批次张量 $(B,L,d)$ 的三个轴通常是样本、序列位置、特征。`a * b` 的逐元素乘法保留每个位置，结果仍是向量；`a @ b` 的点积会把对应位置乘积求和，结果是一个数；矩阵乘法则批量做点积。程序允许广播不代表语义正确，完整手算见 [3.0.2 张量形状与轴](/notebooks/3_0_2_linear_algebra#tensors-shapes) 和 [逐元素运算与点积](/notebooks/3_0_2_linear_algebra#elementwise-dot)。
"""),
            code(r"""
...
这里的指数与对数并不神秘：$e^z$ 会把加法尺度变成正的倍数尺度，$\log$ 是它的反函数，并把连乘变回相加。概率 $p$ 对应的 **odds** 是 $p/(1-p)$；取对数得到 $\log\frac{p}{1-p}=z$，所以 Sigmoid 正是在把 log-odds 还原成概率。若已知用户属于某人群 $B$，点击概率写成条件概率 $P(Y=1\mid B)$；许多样本的平均结果由期望 $E[Y]$ 描述。详见 [3.0.4 条件概率](/notebooks/3_0_4_probability_statistics#conditional-chain)、[期望与方差](/notebooks/3_0_4_probability_statistics#expectation-variance) 和 [odds、logit 与校准](/notebooks/3_0_4_probability_statistics#likelihood-calibration)。
...
这些激活通常逐元素作用，不改变张量形状；Softmax 则跨一组位置归一化并让它们竞争权重。完整的函数复合见 [3.0.3 函数与复合](/notebooks/3_0_3_calculus#functions)，逐元素/点积对照见 [3.0.2](/notebooks/3_0_2_linear_algebra#elementwise-dot)。
"""),
            md(r"""
## 6. 梯度下降：沿着下坡方向一点点改参数
...
多个参数时，“暂时固定其他参数，只让一个参数变化”的坡度叫**偏导**；把所有偏导排在一起就是梯度。模型是复合函数，所以反向传播从损失端按**链式法则**逐层相乘，并在分叉处相加；它不是另一套数学。详见 [3.0.3 偏导与梯度](/notebooks/3_0_3_calculus#derivative-gradient) 和 [链式法则与反向传播](/notebooks/3_0_3_calculus#chain-rule)。

更新公式 $w\leftarrow w-\eta\nabla L$ 中，$\eta$ 是学习率，像每一步的步长。过大会跨过谷底甚至发散，过小则走得很慢。实际训练常用 **mini-batch**：每次用一小批样本估计梯度，在内存、速度和噪声间折中。训练误差继续下降而验证误差上升叫**过拟合**；L1/L2 正则化、早停与更多可靠数据都是约束方法。详见 [3.0.6 SGD 与 mini-batch](/notebooks/3_0_6_optimization#sgd) 和 [正则化与过拟合](/notebooks/3_0_6_optimization#regularization)。下面在一元二次函数 $L(w)=(w-3)^2+1$ 上亲手走 12 步。
"""),
            code(r"""
learning_rate = 0.18
w = -2.0
...
**通用先修：** [3.0.1 隐式反馈、未知项与假负例](/notebooks/3_0_1_data_ml_basics#implicit-feedback) · [3.0.2 逐元素、点积与余弦](/notebooks/3_0_2_linear_algebra#elementwise-dot) · [3.0.2 矩阵乘法](/notebooks/3_0_2_linear_algebra#matmul-embedding)<br>
...
**通用先修：** [3.0.2 低秩分解与形状](/notebooks/3_0_2_linear_algebra#low-rank-attention) · [3.0.3 偏导与梯度](/notebooks/3_0_3_calculus#derivative-gradient) · [3.0.6 SGD](/notebooks/3_0_6_optimization#sgd) · [3.0.6 L2 与过拟合](/notebooks/3_0_6_optimization#regularization)<br>
...
**通用先修：** [3.0.1 样本、特征与标签](/notebooks/3_0_1_data_ml_basics#observation-label) · [3.0.2 逐元素与点积](/notebooks/3_0_2_linear_algebra#elementwise-dot) · [3.0.5 二元交叉熵](/notebooks/3_0_5_information_theory#cross-entropy-kl)<br>
...
**通用先修：** [3.0.1 标签、曝光与校准边界](/notebooks/3_0_1_data_ml_basics#implicit-feedback) · [3.0.4 odds、logit 与校准](/notebooks/3_0_4_probability_statistics#likelihood-calibration) · [3.0.5 交叉熵与 Normalized Entropy](/notebooks/3_0_5_information_theory#softmax-temperature)<br>
...
""")
    ...
    specs["4_1_openonerec_practice"] = notebook(
        "4.1 OpenOneRec 实战：从 RecIF-Bench 到约束列表生成",
    ...
    specs["4_2_dlrm_hstu_practice"] = notebook(
        "4.2 DLRM HSTU 实战：行为序列生成式推荐",
