...
def dataset_for_title(title: str) -> tuple[str, str]:
    if title.startswith("4.5 GBDT+LR"):
        return "criteo-x1", "MovieLens 评分派生标签（bounded teaching view）；full 档使用 Criteo_x1 官方 7:2:1 切分"
    ...
    if "DeepFM" in title:
        return "criteo-x1", "KuaiRand-Pure（smoke 教学适配器）；full 档使用 Criteo_x1 官方 7:2:1 切分"
    if "OpenOneRec" in title:
        return "openonerec-recif", "KuaiRand Semantic-ID（smoke 教学适配器）；full 档读取授权的 RecIF-Bench 本地副本"
    if "HSTU" in title:
        return "movielens-20m", "KuaiRand feed 序列（smoke 教学适配器）；full 档使用 MovieLens 20M 公开基准"
...
""")
    ...
    def classic_runner(slug: str):
        return code(f"""
from importlib import import_module
from recsys_lab.runtime import print_progress

chapter_train = import_module("chapter_code.{slug}.train")
...
""")] + [source_cells[4], bounded_classic_data] + [source_cells[i] for i in [6, 7, 8]] + [
            md(r"""
...
""")
            ...
            classic_runner_intro,
            classic_runner("4_2_collaborative_filtering"),
            md("## Checks\n\nbounded teaching view 检查相似度矩阵、已见物品屏蔽与指标范围；正式产物另行检查 runner 的反造假字段。"),
...
usercf_value = result.get("usercf_hr@10") if PROFILE == "smoke" else None
itemcf_value = result.get("itemcf_recall@5", result.get("recall@5"))
payload = {"records": [
    {"algorithm":"UserCF", "task":"Top-K", "metric":"HR@10 ↑", "value":usercf_value,
     "secondary_metric":"Coverage", "secondary_value":result.get("usercf_coverage"),
     "status":"not_run_full_dense_usercf" if PROFILE == "full" else ("executed" if usercf_value is not None else "not_reported_by_runner"),
     "online_inference":"聚合相似用户的历史", "source_notebook":"4_2_collaborative_filtering"},
    {"algorithm":"ItemCF", "task":"Top-K", "metric":"Recall@5 ↑", "value":itemcf_value,
     "secondary_metric":"Coverage", "secondary_value":result.get("itemcf_coverage", result.get("coverage")),
     "status":"executed", "online_inference":"聚合历史物品的近邻", "source_notebook":"4_2_collaborative_filtering"},
]}
...
""")] + [source_cells[4], bounded_classic_data] + [source_cells[i] for i in [6, 7]] + [
            ...
            classic_runner_intro,
            classic_runner("4_3_matrix_factorization"),
...
"""), bounded_classic_data] + [source_cells[i] for i in [20, 21, 22, 23, 24, 25, 26, 27]] + [
            classic_runner_intro,
            classic_runner("4_4_factorization_machine"),
...
"""), bounded_classic_data] + [source_cells[i] for i in [20, 21, 22, 28, 29, 30, 31, 32, 33, 34, 35]] + [
            classic_runner_intro,
            classic_runner("4_5_gbdt_lr"),
...
"""),
            classic_runner_intro,
            classic_runner("4_6_word2vec"),
            md("## Checks\n\nbounded teaching view 确认序列非空、loss 下降与 Recall 范围；正式产物来自 runner。"),
            code(r"""
assert len(pairs) > 0
assert losses[-1] < losses[0]
...
result_dir = ARTIFACT_ROOT / "results" / "chapter_4"; result_dir.mkdir(parents=True, exist_ok=True)
payload = {"records": [
    {"algorithm": "word2vec / Item2Vec", "task": "Top-K", "metric": "Recall@5 ↑", "value": result["recall@5"],
     "secondary_metric": "Coverage", "secondary_value": result.get("coverage"),
     "status": "executed", "online_inference": "物品向量 ANN 召回",
     "source_notebook": "4_6_word2vec"},
]}
