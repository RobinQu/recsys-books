...
MATH_BY_HAND = {
    "5_2_dssm": (
        "**通用先修：** 3.3 余弦相似度、3.5 Sigmoid 与 3.6 Softmax/温度。"
    ...
    "5_3_mind": (
        "**通用先修：** 3.3 加权汇总、向量模长与张量形状，3.6 Softmax。"
    ...
    "5_4_sasrec": (
        "**通用先修：** 3.3 点积注意力与形状，3.6 Softmax 温度。"
    ...
    "6_2_deepfm": (
        "**通用先修：** 3.3 点积、3.5 Sigmoid 与 3.6 二元交叉熵。"
    ...
    "7_3_ple": (
        "**通用先修：** 3.6 Softmax、3.7 梯度冲突，以及上节 MMoE。"
    ...
    "8_2_openonerec_practice": (
        "**通用先修：** 3.5 条件概率与 3.6 Softmax/序列 NLL。"
...
def patch_math_foundations() -> None:
    path = NB_DIR / "3_1_math_foundations.ipynb"
    nb = nbformat.read(path, as_version=4)
    if any("### 5.1 逐元素运算" in c.source for c in nb.cells if c.cell_type == "markdown"):
        print("3_1_math_foundations already has §5.1, skipping")
        return
    ...
    print(f"inserted §5.1 at index {idx} in 3_1_math_foundations")
