...
- **3.0 总览与实验管线**：`3_1_math_foundations` 给出共同路线和最小手算直觉，`3_8_data_pipeline` 串起数据切分、张量化、训练、推理、测试与结果落盘。
- **3.2—3.7 共性课程**：6 个可独立学习的 Notebook，依次负责数据/机器学习基础、线性代数、微积分、概率统计、信息论与损失、优化。稳定锚点供算法页精确回链。
...
| slug | 初审 | 已处理的缺口 |
|---|---:|---|
| `4_2_collaborative_filtering` | P2 | 补相似度/邻域手算与通用向量链接 |
| `4_3_matrix_factorization` | P2 | 补低秩单格重构、损失到更新与优化链接 |
| `4_4_factorization_machine` | A | 固化二阶恒等式、符号和数据任务边界 |
| `4_5_gbdt_lr` | P1 | 补叶编码→LR 公式，并隔离 Facebook NE 与 MovieLens 教学结果 |
| `4_6_word2vec` | P2 | 补负采样逐项代入、未知项语义与信息论链接 |
| `5_2_dssm` | A | 固化双塔/余弦/条件概率与搜索论文到推荐迁移边界 |
| `5_3_mind` | P1 | 补动态路由、squash、label-aware attention 及 19:1/时间切分边界 |
| `5_4_sasrec` | A | 固化 Q/K/V、因果遮罩、逐位置损失与候选协议 |
| `6_2_deepfm` | A | 固化 FM 化简、共享 embedding 和 Criteo/KuaiRand 边界 |
| `6_3_din` | P1 | 补未归一化激活、Dice/MBA 正则，并隔离阿里广告与 KuaiRand 指标 |
| `6_4_dien` | A | 固化辅助损失、GRU/AUGRU 与严格时序语义 |
| `7_2_mmoe` | A | 固化任务 gate、张量形状和共享梯度路径 |
| `7_3_ple` | A | 固化 CGC、渐进抽取、样本空间与任务权重 |
| `8_2_openonerec_practice` | P1 | 补自回归/DPO/trie 推导，并隔离官方 RecIF/reward 与本地 smoke |
| `8_3_dlrm_hstu_practice` | P1 | 补非 Softmax 的 SiLU 聚合，并隔离 Meta full 协议与 KuaiRand 适配器 |
...
- `app/content.py`：Notebook 角色、第 3 章课程 manifest、模型到课程覆盖关系。
